(function(root, factory) {
    ...
    var loadDependence = function(fncallback) {
        ...
        if (!window.wwload.offline) {
            window.wwload.offline = "wait";
            requirejs.config({
                paths: {
                    "offline": "libs/offline/offline" // 
                }
            });
            require(["offline"], function() { // css!libs/offline-language-chinese-simplified.css
                require([""], function() {
                    window.wwload.offline = true;
                    // window.Offline = Offline;
                    replace();
                    fncallback();
                });
            });
        } else if (window.wwload.offline === "wait") {
            setTimeout(function() {
    ...
    function processElement($ele) {
        ...
        if ($ele.attr("data-intline")=="true") {
            Offline.check();
            if (Offline.state == "up") {
                // console.log('online事件');
                $.wwclass.helper.updateProp($ele, "data-x-line", "true");
                $.wwclass.helper.anijsTrigger($ele, "online");
                $ele.attr("data--line", "false");
            };
            if (Offline.state == "down") {
                // console.log('offline事件');
                $.wwclass.helper.updateProp($ele, "data-x-line", "false");
                $.wwclass.helper.anijsTrigger($ele, "offline");
            };
        };
        setTimeout(function() {
            $.wwclass.helper.updateProp($ele, "data-x-inited", true);
    ...
    var evtInHandler = function($ele, attribute, value) {
        switch (attribute) {
            case "data--line":
                ...
                if (value == "true") {
                  Offline.check();
                  var lineTime =  Number($ele.attr("data-linetime"));
                    setTimeout(function() {
                        Offline.check();
                        ...
                        if (Offline.state == "down") {
                            ...
                            $.wwclass.helper.updateProp($ele, "data-x-line", "false");
                            $.wwclass.helper.anijsTrigger($ele, "offline");
                            var lineState = setInterval(function() {
                                Offline.check();
                                // console.log("-------:" + JSON.stringify(Offline));
                                if (Offline.state == "up") {
                                    clearInterval(lineState);
                                    // console.log('online事件');
                                    $.wwclass.helper.updateProp($ele, "data-x-line", "true");
                                    $.wwclass.helper.anijsTrigger($ele, "online");
                                    $ele.attr("data--line", "false");
